...
Runs the standalone high-performance FastAPI server and closed-loop autonomous
trading system across the 6 canonical modules:
1. core
2. scanner
3. execution
4. telegram
...
async def lifespan(app: FastAPI):
    """FastAPI lifespan: startup then graceful shutdown."""
    ...
    logger.info("PROJECT-ALPHA starting", extra={"port": cfg.port, "db": cfg.db_path})
    ...
    # 7. Production Controller & 24/7 Watchdog Supervisor
    ...
    logger.info("PROJECT-ALPHA startup complete — background scanner warm-up triggered")

    yield  # Application is running

    # ── Graceful Shutdown ─────────────────────────────────────────────────────
    logger.info("PROJECT-ALPHA shutting down")
    ...
    if _db:
        await _db.close()
    logger.info("PROJECT-ALPHA shutdown complete")


# ── FastAPI Application ───────────────────────────────────────────────────────
app = FastAPI(
    title="PROJECT-ALPHA",
    description="Autonomous Multi-Strategy Algorithmic Trading Infrastructure",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
# API routes mounted at canonical /api and backward-compatible /api/v2
